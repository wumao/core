...
class AqaraLightEntityDescription(LightEntityDescription):
    ...
    brightness_res_id: str = "14.7.1006"
    brightness_max_res_id: str = ""
    brightness_min_res_id: str = ""

    rgbw_color_res_id: str = ""
    rgb_color_res_id: str = ""

    color_data: float | tuple[float, ...] | None = None

    # color temperature setting
    color_temp_res_id: str = ""
    coldest_color_temperature: int = 150
    warmest_color_temperature: int = 370
...
def rgbw_to_argb(rgb: tuple[int, int, int, int]) -> int:
    argb = (
...
class AqaraLightEntity(AqaraEntity, LightEntity):
    ...
    def turn_on(self, **kwargs: Any) -> None:
        ...
        if kwargs.get("brightness") is not None:
            x = kwargs.get("brightness")
            brightness = x if x is not None else 0
            commands = {
                self.entity_description.brightness_res_id: str(
                    round(int(brightness) * 100 / 255)
                )
            }
        elif kwargs.get("rgbw_color") is not None:
            x = kwargs.get("rgbw_color")
            rgbw: tuple[int, int, int, int] = x if x is not None else {0, 0, 0, 0}
            argb = rgbw_to_argb(rgbw)
            commands = {self.entity_description.rgbw_color_res_id: str(argb)}
        elif kwargs.get("rgb_color") is not None:
            x = kwargs.get("rgb_color")
            rgb: tuple[int, int, int] = x if x is not None else {0, 0, 0}
