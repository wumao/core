...
class AqaraClimateEntityDescription(ClimateEntityDescription):
    ...
    current_model_range_res_id: str | None = ""
    # 当前匹配的模式范围模式定义：0:制冷，1:制热，2:自动，3:送风
    # 数据格式：string hex
    ...
    current_temperature_range_res_id: str | None = ""
    # 当前匹配的温度范围
    # 数据格式：string hex
    # 字节顺序：byte0：模式0，byte1：模式0最低温度，byte2：模式0最高温度，
    # byte3：模式1，byte4：模式1最低温度，byte5：模式1最高温度 ...
