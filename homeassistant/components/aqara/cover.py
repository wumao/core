...
class AqaraCoverEntityDescription(CoverEntityDescription):
    ...
    # )
    curtain_direct_res_id: str | None = None
    # 窗帘的方向属性 1: 左开 2: 右开 3: 两边开 4卷帘，5柔纱帘，6百叶帘，
    # 7香格里拉帘，8斑马帘，9垂直百叶帘（左开），10垂直百叶帘（右开）
...
class AqaraCoverEntity(AqaraEntity, CoverEntity):
    ...
    def current_cover_position(self) -> int | None:
        ...
        value = self.device_manager.get_point_value(
            self.point.did,
            self.entity_description.curtain_open_percentage_res_id,
        )
        if value == "":
            return None
        return int(value)
